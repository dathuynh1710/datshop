        ...
        <tbody>
            @foreach ($groupedAclRoleHasPermissions as $key => $grhp)
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ route('backend.acl_role_has_permissions.create', ['role_id' => $role_id]) }}"
                            class="btn btn-warning">Hiệu chỉnh quyền cho vai trò</a>

                        <button type="button" class="btn btn-danger btn-delete" data-id="{{ $role_id }}"
                            data-delete-url="{{ route('backend.acl_role_has_permissions.store') }}">Thu hồi tất cả các quyền
                            đã cấp cho vai trò</button>
                    </td>
                </tr>
            @endforeach
        </tbody>

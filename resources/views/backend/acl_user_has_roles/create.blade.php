...
@section('main-content')
    ...
    <form name="frmCreate" method="post" action="{{ route('backend.acl_user_has_roles.store') }}">
        ...
        <div>
            ...
            <select name="user_id" class="form-control">
                @foreach ($lstAclUsers as $u)
                    <option value="{{ $u->id }}">({{ $u->username }}) {{ $u->last_name }} {{ $u->first_name }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            @foreach ($lstRoles as $i => $r)
                <div class="form-check mb-2">
                    <input class="form-check-input" type="checkbox" name="role_id[]" id="role_id_{{ $i }}"
                        value="{{ $r->id }}">
        ...
        <div class="mt-2">
            <a href="{{ route('backend.acl_user_has_roles.index') }}"
